...
# Store OTPs and Resend tracking in memory
otp_store = {
    'current': None,
    'verified': False,
    'resend_count': 0,
    'last_send_time': 0,
    'expiry': 0,
    'session_expiry': 0 # When the verified session expires
}
...
def request_otp():
    now = time.time()
    time_since_last = now - otp_store['last_send_time']
    required_wait = 30 * (2 ** (otp_store['resend_count'] if otp_store['resend_count'] < 5 else 5))
    if otp_store['last_send_time'] == 0: required_wait = 0

    if time_since_last < required_wait:
        return jsonify({'success': False, 'error': f'Wait {int(required_wait - time_since_last)}s', 'next_resend_seconds': int(required_wait - time_since_last)}), 429

    otp = ''.join(random.choices(string.digits, k=6))
    otp_store['current'] = otp
    otp_store['last_send_time'] = now
    otp_store['expiry'] = now + 300
    otp_store['resend_count'] += 1
    
    if send_otp_email(GMAIL_USER, otp):
        next_wait = 30 * (2 ** (otp_store['resend_count'] if otp_store['resend_count'] < 5 else 5))
...
def verify_otp():
    data = request.json
    now = time.time()
    if otp_store['expiry'] < now:
        return jsonify({'success': False, 'error': 'OTP Expired'}), 401
    if data.get('otp') == otp_store.get('current'):
        otp_store['verified'] = True
        otp_store['resend_count'] = 0
        otp_store['session_expiry'] = now + SESSION_DURATION
        return jsonify({'success': True, 'session_expiry': int(otp_store['session_expiry'])})
    return jsonify({'success': False, 'error': 'Invalid OTP'}), 401

@app.route('/api/status', methods=['GET'])
def get_status():
    # If session expired, don't show status (optional but safer)
    if otp_store['session_expiry'] < time.time():
        otp_store['verified'] = False
...
def start_service(service_key):
    if not otp_store.get('verified') or otp_store['session_expiry'] < time.time():
        otp_store['verified'] = False
        return jsonify({'error': 'Unauthorized / Session Expired'}), 403
        
    if service_key not in SERVICES: return jsonify({'error': 'Unknown service'}), 404
    
    config = SERVICES[service_key]
    if is_port_in_use(config['port']): return jsonify({'message': 'Already running'})

...
function AdminLayout({ children }) {
    ...
    const navLinks = [
        ...
        { path: '/admin/categories', icon: Search, label: 'Categories', allowedRoles: ['super_admin', 'admin', 'inventory_admin'] },
        { path: '/admin/delivery', icon: MapPin, label: 'Delivery', allowedRoles: ['super_admin', 'admin', 'delivery_admin'] },
    ...
    return (
        <div className="flex h-screen overflow-hidden bg-gray-50/50">
            {/* Desktop Sidebar — fixed width, full-height scroll */}
            <aside className={`fixed inset-y-0 left-0 z-50 w-64 bg-slate-900 text-white transform transition-transform duration-300 ease-in-out md:translate-x-0 md:static flex flex-col ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
                <div className="flex items-center justify-between p-4 border-b border-slate-700 h-[var(--app-header-height)]">
                    <Link to="/admin" className="text-xl font-bold tracking-tight text-white flex items-center gap-2">
                        <Shield className="text-primary w-6 h-6" /> JDLX Admin
                    </Link>

...
import {
    ...
    FileText, RefreshCw, ShieldCheck, UserRoundCheck, XCircle, Truck
} from 'lucide-react'
...
const UPLOAD_ACCEPT_DOCS = '.jpg,.jpeg,.png,.webp,.pdf'
...
function WarehouseRequest() {
    ...
    useEffect(() => {
        ...
        } catch (oauthParseError) {
            setError('Google verification failed. Please try again.')
            console.error('Warehouse request oauth parsing failed:', oauthParseError)
        }
    ...
    const validateUpload = (file, label, allowDocs = false) => {
        if (!file) return ''
        const ext = `.${(file.name.split('.').pop() || '').toLowerCase()}`
        const allowedExts = allowDocs ? DOC_EXTENSIONS : IMAGE_EXTENSIONS
        if (!allowedExts.includes(ext)) {
            return allowDocs
                ? `${label} must be JPG, JPEG, PNG, WEBP, or PDF.`
    ...
    const handleOwnerImageChange = (e) => {
        ...
        const validationError = validateUpload(file, 'Owner image')
        if (validationError) {
        ...
        setError('')
        setUploads((prev) => ({ ...prev, ownerImage: file || null }))
    }

    const handleKycDocumentChange = (e) => {
        const file = e.target.files?.[0]
        const validationError = validateUpload(file, 'KYC document', true)
        if (validationError) {
            setError(validationError)
            e.target.value = ''
            return
    ...
    const handleStoreImagesChange = (e) => {
        ...
        if (invalid) {
            setError(validateUpload(invalid, 'Store image'))
            e.target.value = ''
            return
        }
        setError('')
        setUploads((prev) => ({ ...prev, storeImages: files }))
    }
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            uploads.storeImages.forEach((storeImage) => {
                payload.append('warehouse_images', storeImage)
            })
            ...
            <div style={{ background: 'rgba(16,185,129,0.08)', border: '1px solid rgba(16,185,129,0.2)', borderRadius: '16px', padding: '20px' }}>
                <div style={{ display: 'flex', gap: '12px' }}>
                    ...
                    <div>
                        ...
                        <Link to="/" style={{ display: 'inline-flex', alignItems: 'center', gap: '8px', background: '#34d399', color: '#0f172a', padding: '10px 18px', borderRadius: '10px', textDecoration: 'none', fontWeight: 700, fontSize: '14px' }}>
        ...
        <div style={{
            ...
            <div style={{ maxWidth: '1000px', margin: '0 auto' }}>
                ...
                <div style={{ marginBottom: '24px', display: 'flex', gap: '12px' }}>
                    <div style={{ flex: 1, padding: '2px', background: 'linear-gradient(90deg, #14b8a6, transparent)', borderRadius: '14px' }}>
                        <div style={{ background: '#0f172a', padding: '16px 20px', borderRadius: '12px', display: 'flex', alignItems: 'center', gap: '12px' }}>
                            <div style={{ width: '32px', height: '32px', borderRadius: '8px', background: 'rgba(20,184,166,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#14b8a6' }}>
                                <FileText size={16} />
                            </div>
                            <span style={{ fontWeight: 700, fontSize: '14px' }}>Warehouse Application</span>
                        </div>
                    </div>
                    <Link to="/warehouse/request-delivery" style={{ flex: 1, textDecoration: 'none', color: 'inherit' }}>
                        <div style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.05)', padding: '16px 20px', borderRadius: '14px', display: 'flex', alignItems: 'center', gap: '12px', opacity: 0.6 }}>
                            <div style={{ width: '32px', height: '32px', borderRadius: '8px', background: 'rgba(255,255,255,0.05)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#94a3b8' }}>
                                <Truck size={16} />
                            </div>
                            <span style={{ fontWeight: 600, fontSize: '14px' }}>Delivery Partner</span>
                        </div>
                    </Link>
                </div>
                    ...
                    <div style={{
                        ...
                        <div>
                            ...
                            <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap' }}>
                                ...
                                <Link to="/" style={{ display: 'inline-flex', alignItems: 'center', gap: '8px', background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.1)', color: '#cbd5e1', padding: '14px 22px', borderRadius: '12px', textDecoration: 'none', fontSize: '14px', fontWeight: 600 }}>
                                ...
                                <form onSubmit={handleSubmit}>
                                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '14px' }}>
                                        ...
                                        <div style={{ gridColumn: '1/-1', display: 'flex', flexDirection: 'column', gap: '6px' }}>
                                            ...
                                            <textarea
                                                name="request_mail_message"
                                                value={formData.request_mail_message}
                                                onChange={handleInputChange}
                                                rows={4}
                                                placeholder="Type the request mail you want admin team to review..."
                                                style={{ ...s.input, resize: 'vertical' }}
                                            />
                                        </div>
                                        <div style={{ gridColumn: '1/-1', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '14px', padding: '14px', display: 'grid', gap: '12px' }}>
                                            <p style={{ margin: 0, fontSize: '12px', fontWeight: 700, color: '#14b8a6', textTransform: 'uppercase', letterSpacing: '0.08em' }}>
                                                Additional Uploads
                                            </p>
                                            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                                                <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                                                    <label style={s.label}>Owner Image</label>
                                                    <input type="file" accept={UPLOAD_ACCEPT_IMAGES} onChange={handleOwnerImageChange} style={s.fileInput} />
                                                    {uploads.ownerImage && (
                                                        <p style={{ margin: 0, fontSize: '11px', color: '#67e8f9' }}>{uploads.ownerImage.name}</p>
                                                    )}
                                                </div>
                                                <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                                                    <label style={s.label}>KYC Document</label>
                                            ...
                                            <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                                                <label style={s.label}>Store Images (Multiple)</label>
                                                <input type="file" accept={UPLOAD_ACCEPT_IMAGES} multiple onChange={handleStoreImagesChange} style={s.fileInput} />
                                                {uploads.storeImages.length > 0 && (
                                                    <p style={{ margin: 0, fontSize: '11px', color: '#67e8f9' }}>
                                                        {uploads.storeImages.length} image(s) selected
                                                    </p>
                                                )}
                                            </div>
                                        </div>
